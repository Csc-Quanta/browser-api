...
public class AddressHelper implements ActorService {
	...
	public AddressInfo getAccountByAddress(String address){
		...
		actTokenValues.forEach(tokenValue->{
			...
			token.setBalance(NumUtil.dealZero(zcBcActTokenValue.getActvBalance().toString(),8));
			token.setLocked(NumUtil.dealZero(zcBcActTokenValue.getActvLocked().toString(),8));
